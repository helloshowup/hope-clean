...
async def extract_student_handbook_information(content_outline: str, handbook_path: str, ui_settings: Dict[str, Any]) -> str:
    ...
    try:
        ...
        try:
            ...
            # Prepare the handbook for vector search
            ...
            # Index textbook on demand (reuses existing index if unchanged)
            try:
                db.index_textbook(handbook_text, textbook_id)
            except Exception:
                logger.exception("Handbook vectorization failed")
                raise
            ...
            # Convert results to plain strings if dictionaries were returned
            relevant_chunks = [
            ...
            logger.exception("Error in RAG extraction:")
            return f"Error extracting relevant information: {str(e)}"
        
    except Exception as e:
        logger.exception("Error extracting handbook information:")
        ...
        for i, row in enumerate(csv_rows):
            ...
            try:
                ...
                row_data.append({
                    ...
                    "row": row,
                    "variables": variables,
                    "word_count": variables.get("word_count"),
                    "template": template,
