...
async def edit_content(
    ...
    for indicator in error_indicators:
        if indicator.lower() in content.lower():
            logger.warning(
                f"Content appears to be an AI response rather than educational content: '{indicator}'"
            )
            return (
                content,
                "Content appears to be an AI response rather than educational content. No edits performed.",
            )
    ...
    try:
        ...
        logger.info(
            f"Extracted edited content ({len(edited_content)} characters) and explanation"
        )
...
def _load_ai_phrases() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Loaded AI phrases data: {pattern_count} pattern categories, {phrase_count} individual phrases"
        )
...
def _create_editing_prompt(
    content: str, detected_patterns: Dict[str, Any], target_learner: str
) -> str:
    """
    ...
    if not patterns_info:
        patterns_info = (
            "No specific patterns provided, but the text has AI-like characteristics."
        )
    ...
    try:
        ...
        logger.info(
            f"Created editing prompt from template loader ({len(prompt)} characters)"
        )
        return prompt
    except ImportError:
...
def _extract_editing_results(result: str) -> Tuple[str, str]:
    ...
    edited_text_match = re.search(
        r"<edited_text>(.*?)</edited_text>", result, re.DOTALL
    )
...
def run_ai_detection_stage(
    content: str, patterns_file: Optional[str] = None
) -> List[Dict[str, Any]]:
    ...
    for cat in patterns_data.get("patterns", []):
        ...
        for pattern_entry in cat.get("patterns", []):
            # allow either a raw regex string or an object with 'pattern'
            if isinstance(pattern_entry, dict):
                pattern = pattern_entry.get("pattern") or pattern_entry.get("regex")
                if not pattern:
                    continue
                min_occ = int(pattern_entry.get("min_occurrences", 1))
            else:
            ...
            occurrences = list(regex.finditer(content))
