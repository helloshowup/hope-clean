...
class ClaudeAIPanel(ttk.Frame):
    ...
    def __init__(self, parent, main_app, *args, **kwargs):
        ...
        # Initialize model variables with fixed models
        self.context_model = "claude-3-haiku-20240307"  # Fixed model for context generation
        self.edit_model = "claude-3-7-sonnet-20250219"  # Fixed model for edits and complex prompts
    ...
    def _populate_library(self, directory=None):
        ...
        if directory is None:
            # Use the specified library directory
    ...
    def _load_profiles(self):
        """Load system profiles from profile directory"""
        self.profiles = {}
    ...
    def _setup_file_tree_context_menu(self):
        ...
        # Add menu items
        self.file_tree_menu.add_command(label="Open", command=self._open_selected_file)
    ...
    def add_text_editor_tool_button(self, parent_frame):
        ...
        """
        text_editor_frame = ttk.LabelFrame(parent_frame, text="Token-Efficient Editing (Claude 3.7 Sonnet)")
    ...
    def get_selected_files(self):
        """Get the list of selected files from the file tree.
        
        Returns:
            list: A list of full paths to the selected files
        """
        selected_files = []
        selected_items = self.file_tree.selection()
        ...
        for item_id in selected_items:
            ...
            if os.path.isfile(full_path):
                ...
                # For directories, find all markdown files within them
...
class CacheManager:
    def setup_cache_tab(self):
        # Create a frame for the cache tab
        cache_frame = ttk.LabelFrame(self.parent, text="Cache")
        cache_frame.pack(fill="both", expand=True, padx=10, pady=5)
        
        # Add a label for the cache tab to explain the options
        ttk.Label(cache_frame, text="Claude's Native Caching vs. Local Cache").pack(pady=5, padx=10, anchor="w")
        
        # Add an explanation about the two caching systems
        cache_explanation = ttk.Label(cache_frame, text="The app has two caching systems:\n" + 
                                    "1. Claude's Native Caching: Managed by Anthropic, reduces token usage\n" + 
                                    "2. Local Cache: Stores responses locally to avoid duplicate API calls\n\n" + 
                                    "Using both systems is redundant. You can disable the local cache to rely solely on Claude's native caching.", 
                                    justify="left", wraplength=550)
        cache_explanation.pack(pady=5, padx=10, anchor="w")
        
        # Add a checkbox to enable/disable local cache
        self.use_local_cache_var = tk.BooleanVar(value=True)  # Default to using local cache
        use_local_cache_checkbox = ttk.Checkbutton(cache_frame, 
                                                 text="Use Local Cache (redundant with Claude's native caching)", 
                                                 variable=self.use_local_cache_var,
                                                 command=self.parent.toggle_local_cache)
        use_local_cache_checkbox.pack(pady=5, padx=10, anchor="w")

    def _send_to_batch_edit(self):
        """Send selected files to the batch edit tab for processing."""
        selected_items = self.file_tree.selection()
        if not selected_items:
            messagebox.showinfo("No Selection", "Please select files to process.")
            return
        ...
        # Update batch processor with the files
        if hasattr(self, "batch_processor"):
            self.batch_processor.set_batch_files(batch_files)
            
        logger.info(f"Sent {len(batch_files)} files to batch processing")
