...
* **Persisted Model Selection with OpenAI Support:** The application now loads your preferred model from `user_settings.json` when it starts, falling back to the default model if none is found. Both the main processing model and the initial generation model are saved back to this file, and all generation steps respect these selections. When an OpenAI model is chosen and an API key is provided, the workflow seamlessly uses the OpenAI API instead of Claude.
